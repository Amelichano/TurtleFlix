...
import { getDBMovieDetails } from '../api/dbMovies';
...
function MovieDetails({ id }) {
  ...
  useEffect(() => {
    const fetchDbDetails = async () => {
      try {
        const dbData = await getDBMovieDetails(id);
        setDbDetails(dbData);
        setTmdbId(dbData.tmdbId);  
      } catch (error) {
        console.error('DB details 오류 발생 :', error);
        setIsLoading(false);
      }
    };
    ...
    <Card className="mt-6 w-full flex flex-row">
      ...
      <CardBody className="w-2/3">
        ...
        <p className="font-sans text-base font-light">
          <strong>평점:</strong> {dbDetails.averageRating ? `${dbDetails.averageRating} 점` : '정보 없음'}
        </p>
        <p className="font-sans text-base font-light">
